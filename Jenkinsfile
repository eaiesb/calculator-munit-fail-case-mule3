...
stage('upload to atifactory') {
  ...
  "files": [
    {
      "pattern": "**/*.jar",
      "target": "generic-local/mule4-demo-jars_$BUILD_NUMBER/calculator-munit-mule3.jar"
    }
 ]
}"""                 
              def buildInfo1 = server.upload spec: uploadSpec
                      slackSend (color: "#FFA500",message: 'calculator-munit-mule3 Artifacts Uploaded Sucessfully')
            }
    }
}  	
}
   post {
      failure {
           ...
           slackSend (color: "#FF0000",message: 'calculator-munit-mule3 Deployment Failed')
        }
      success {
...
''', subject: 'Jenkins ${BUILD_STATUS} [#${BUILD_NUMBER}] - ${PROJECT_NAME} ${ENV, var="GIT_URL"}', to: 'devops@eaiesb.com'    
          slackSend (color: "#32CD32", message: 'calculator-munit-mule3 Deployment is Sucessful')
        }
  }
}
// steps
